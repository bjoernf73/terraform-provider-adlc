...
# Reads and parses a .pol file into an array of GPRegistryPolicy entries.
    ...
    foreach ($entry in $Entries) {
        if (IsRegistryKey -Path $entry -Hive $Hive) {
            ...
            if (IsRegistryKey -Path $ParentKey -Hive $Hive) {
                # $entry is a property: [key;value;type;size;data]
...
# Serializes one GPRegistryPolicy entry into the [key;value;type;size;data] byte layout.
...
# Appends an array of registry policy entries to an existing .pol file.
...
# Creates a new, empty .pol file with a valid Group Policy Registry file header.
