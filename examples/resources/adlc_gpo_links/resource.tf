...
resource "adlc_backup_gpo" "screen_lock" {
...
resource "adlc_gpo_links" "servers" {
  target            = adlc_organizational_unit.servers.path
  block_inheritance = true

  links = [
    {
      gpo = adlc_backup_gpo.screen_lock.id
    },
    {
      gpo      = adlc_backup_gpo.baseline_workstation.id
      enforced = true
    },
  ]
}
